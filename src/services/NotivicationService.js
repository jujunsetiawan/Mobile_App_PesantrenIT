...
export default class NotifService {
  ...
  scheduleNotif(soundName, schedule) {
    this.lastId++;
    PushNotification.localNotificationSchedule({
      date: new Date(Date.now() + (new Date(`${new Date().toDateString()} ${schedule}:00`).getTime() - new Date().getTime())), // in 30 secs
      channelId: soundName ? 'sound-channel-id' : 'default-channel-id',
      ticker: 'My Notification Ticker', // (optional)
      autoCancel: true, // (optional) default: true
      largeIcon: 'ic_launcher', // (optional) default: "ic_launcher"
      smallIcon: 'ic_notification', // (optional) default: "ic_notification" with fallback for "ic_launcher"
      bigText: 'Sudah memasuki waktu sholat, tinggalkan pekerjaan mu dan mari kita tunaikan sholat.', // (optional) default: "message" prop
      subText: `${new Date().getHours()}:${new Date().getMinutes()}`, // (optional) default: none
      color: '#425F57', // (optional) default: system default
      vibrate: true, // (optional) default: true
      ...
      timeoutAfter: null, // (optional) Specifies a duration in milliseconds after which this notification should be canceled, if it is not
      id: this.lastId, // (optional) Valid unique 32 bit integer specified as string. default: Autogenerated Unique ID
      title: 'Sholat Time', // (optional)
      message: 'Sudah memasuki waktu sholat, tinggalkan pekerjaan mu dan mari kita tunaikan sholat.', // (required)
